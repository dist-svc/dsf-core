...
impl rand_core_0_5::RngCore for RandHelper {
    ...
    fn try_fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), rand_core_0_5::Error> {
        match self.0.try_fill_bytes(dest) {
            Ok(()) => Ok(()),
            Err(e) => {
                let code = e.code().unwrap_or(rand_core_0_5::Error::CUSTOM_START.try_into().unwrap());
...
impl SecKey for RustCrypto {
    ...
    fn new_sk() -> Result<SecretKey, Self::Error> {
        ...
        let secret_key = SecretKey::try_from(key.deref())
            .map_err(|_| () )?;
    ...
    fn sk_encrypt(secret_key: &SecretKey, assoc: Option<&[u8]>, message: &mut [u8]) -> Result<SecretMeta, Self::Error> {
        ...
        let tag = cipher.encrypt_in_place_detached(&nonce, assoc.unwrap_or(&[]), message)
            .map_err(|e| {
    ...
    fn sk_decrypt(secret_key: &SecretKey, meta: &[u8], assoc: Option<&[u8]>, message: &mut [u8]) -> Result<(), Self::Error> {
        ...
        cipher.decrypt_in_place_detached(&nonce, assoc.unwrap_or(&[]), message, &tag)
            .map_err(|_| () )?;
...
impl Hash for RustCrypto {
    ...
    fn kdf(key: Array32) -> Result<Array32, ()> { 
        ...
        let mut inst = blake2::Blake2bMac::<U32>::new_with_salt_and_personal(&key, &salt, &DSF_NS_KDF_CTX)
            .map_err(|_| () )?;
...
mod test {
    ...
    fn test_sk_encrypt_decrypt() {
        ...
        let meta = RustCrypto::sk_encrypt(&secret, None, &mut message).expect("Error encrypting data");
        assert!(data != message);
    ...
    fn bench_pk_sign(b: &mut Bencher) {
        let (_public, private) = RustCrypto::new_pk().expect("Error generating public/private keypair");
        let data = [0xabu8; 256];
    ...
    fn bench_pk_sk_convert(b: &mut Bencher) {
        let (pub_key_a, pri_key_a) = RustCrypto::new_pk().expect("Error generating public/private keypair");
        let (pub_key_b, _pri_key_b) = RustCrypto::new_pk().expect("Error generating public/private keypair");

        b.iter(|| {
            let _ =
                RustCrypto::kx(&pub_key_a, &pri_key_a, &pub_key_b).expect("Error deriving secret keys");
        });
    }
    ...
    fn bench_sk_decrypt(b: &mut Bencher) {
        ...
        let sig = RustCrypto::sk_encrypt(&sec_key, None, &mut data)
